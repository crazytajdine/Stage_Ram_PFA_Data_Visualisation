...
try:
    ...
    from data_managers.db import get_engine
...
class AuthDatabaseManager:
    ...
    def _init_db(self) -> None:
        with self.engine.begin() as conn:
            ...
            try:
                ...
                count = conn.execute(text(f"SELECT COUNT(*) FROM {self.users_table}")).scalar_one()
                if not count:
    ...
    def _ensure_role_exists(self, conn, role: str, created_by: str = "system") -> None:
        ...
        if not r:
            return
        exists = conn.execute(text(f"SELECT 1 FROM {self.roles_table} WHERE role=:r"), {"r": r}).first()
        if not exists:
            conn.execute(text(f"INSERT INTO {self.roles_table} (role, created_by) VALUES (:r, :c)"),
    ...
    def verify_user(self, email: str, password: str) -> bool:
        ...
        with self.engine.begin() as conn:
            row = conn.execute(
                text(f"SELECT password_hash, is_active FROM {self.users_table} WHERE email=:e"),
                {"e": email_norm},
            ).first()
        if not row:
            return False
    ...
    def create_user(self, email: str, password: str, role: str, created_by: str = "system") -> int:
        ...
        with self.engine.begin() as conn:
            ...
            if role == "admin" and self._to_bool(is_active):
                active_admins = conn.execute(
                    text(f"SELECT COUNT(*) FROM {self.users_table} WHERE role='admin' AND is_active")
                    if self.dialect == "postgresql" else
                    text(f"SELECT COUNT(*) FROM {self.users_table} WHERE role='admin' AND is_active=1")
                ).scalar_one()
                if int(active_admins) <= 1:
                    raise ValueError("Cannot delete the last active admin.")
            conn.execute(text(f"DELETE FROM {self.users_table} WHERE id=:i"), {"i": uid})
    ...
    def assign_user_role(self, user_id: int, role: str) -> None:
        ...
        with self.engine.begin() as conn:
            ...
            conn.execute(text(f"UPDATE {self.users_table} SET role=:r WHERE id=:i"),
                         {"r": r, "i": int(user_id)})

    # -------------------- Roles & Permissions API --------------------
    def list_roles(self) -> List[str]:
        with self.engine.begin() as conn:
            rows = conn.execute(text(f"SELECT role FROM {self.roles_table} ORDER BY role ASC")).fetchall()
    ...
    def delete_role(self, role: str) -> None:
        ...
        with self.engine.begin() as conn:
            ...
            conn.execute(text(f"DELETE FROM {self.role_permissions_table} WHERE role=:r"), {"r": r})
            conn.execute(text(f"DELETE FROM {self.roles_table} WHERE role=:r"), {"r": r})
    ...
    def get_role_permissions(self, role: Optional[str]) -> List[str]:
        ...
        with self.engine.begin() as conn:
            rows = conn.execute(
                text(f"SELECT page_slug FROM {self.role_permissions_table} WHERE role=:r ORDER BY page_slug"),
                {"r": r},
            ).fetchall()
    ...
    def set_role_permissions(self, role: str, page_slugs: Sequence[str]) -> None:
        ...
        with self.engine.begin() as conn:
            ...
            for s in slugs:
                conn.execute(
                    text(f"INSERT INTO {self.role_permissions_table} (role, page_slug) VALUES (:r, :s)"),
                    {"r": r, "s": s},
                )
