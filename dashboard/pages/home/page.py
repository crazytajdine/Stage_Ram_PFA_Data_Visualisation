...
layout = dbc.Container(
    [
        html.Div(
            ...
            children=[
                html.H1(
                    "Suivi des Vols - Royal Air Maroc",
...
def search_flight(n_clicks, ac_type, ac_reg, dt_start_str, dt_end_str):
    ...
    try:
        ...
        dt_start = (
            datetime.strptime(dt_start_str, "%Y-%m-%dT%H:%M") if dt_start_str else None
        )
        dt_end = datetime.strptime(dt_end_str, "%Y-%m-%dT%H:%M") if dt_end_str else None

        if ac_type:
            df = df.filter(pl.col("AC_SUBTYPE").str.to_lowercase() == ac_type)
        if ac_reg:
            df = df.filter(pl.col("AC_REGISTRATION").str.to_lowercase() == ac_reg)
        ...
        df_max_retard = (
            df.filter(pl.col("Retard en min") > 0)
            .sort("Retard en min", descending=True)
            .limit(1)
        )
        if df_max_retard.height > 0:
            ...
            somme_retards = (
                df.select(pl.col("Retard en min").sum())
                .to_dicts()[0]
                .get("Retard en min", 0)
            )
        ...
        df_display = df.select(cols).rename(
            {
                "AC_SUBTYPE": "SUBTYPE",
        ...
        mean_15_plus = (
            df.filter(pl.col("Retard en min") >= 15)
            .select(pl.col("Retard en min").mean())
            .to_dicts()[0]
            .get("Retard en min", 0)
            if count_15_plus > 0
            else 0
