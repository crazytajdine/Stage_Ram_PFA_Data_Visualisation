...
def analyze_weekly_codes(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    df_with_days = frame.with_columns(
        pl.col("DEP_DATE")
        .map_elements(get_day_of_week, return_dtype=pl.Utf8)
        .alias("day_of_week")
    )

    # Group by code and day of week
    weekly_data = (
        df_with_days.group_by(["CODE_DR", "day_of_week"])
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            html.H1(
                                "ANALYSE HEBDOMADAIRE DES CODES DE RETARD",
                                className="mb-2",
                            ),
                            html.P(
                                "Analysez la répartition des codes de retard par jour de la semaine.",
                                className="lead mb-4",
                            ),
                        ]
                    ),
...
@app.callback(
    Output("weekly-matricule-dd", "options"), Input("weekly-flotte-dd", "value")
)
