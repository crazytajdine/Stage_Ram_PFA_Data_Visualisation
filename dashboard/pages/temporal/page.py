...
def get_time_period_column(df: pl.DataFrame, segmentation: str) -> pl.DataFrame:
    ...
    if segmentation == "days":
        return df.with_columns(
            pl.col("DEP_DATE").alias("period")
        )
    elif segmentation == "weeks":
        return df.with_columns(
            pl.col("DEP_DATE").dt.strftime("%Y-W%U").alias("period")
        )
    elif segmentation == "months":
        return df.with_columns(
            pl.col("DEP_DATE").dt.strftime("%Y-%m").alias("period")
        )
    else:
        return df
        ...
        children=[
                    ...
                    [
                        dbc.Row(
                            [
                                ...
                                dbc.Col(
                                    [
                                        html.Label("Segmentation :", className="form-label"),
                                        dcc.Dropdown(
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        [
                            html.H3("📊 Distribution totale des retards", className="h4 mb-3"),
                            dbc.Card(
                                dcc.Graph(id="temporal-column-chart", style={"height": "400px"}),
                                className="mb-4",
                            ),
                            
                            html.H3("📈 Évolution des codes de retard", className="h4 mb-3"),
                            dbc.Card(
                                dcc.Graph(id="temporal-line-chart", style={"height": "400px"}),
                            ),
                        ],
                        md=6,
                    ),
