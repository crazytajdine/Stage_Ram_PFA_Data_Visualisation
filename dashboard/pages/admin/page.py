...
layout = dbc.Container(
    [
        ...
        # Stats
        dbc.Row([
            dbc.Col(dbc.Card(dbc.CardBody([html.H4("Total Users", className="text-muted mb-1"),
                                           html.H2(id="total-users-count", className="text-primary mb-0")]),
                             className="text-center"), md=3),
            dbc.Col(dbc.Card(dbc.CardBody([html.H4("Active Users", className="text-muted mb-1"),
                                           html.H2(id="active-users-count", className="text-success mb-0")]),
                             className="text-center"), md=3),
            dbc.Col(dbc.Card(dbc.CardBody([html.H4("Admin Users", className="text-muted mb-1"),
                                           html.H2(id="admin-users-count", className="text-warning mb-0")]),
                             className="text-center"), md=3),
            dbc.Col(dbc.Card(dbc.CardBody([html.H4("Recent Logins", className="text-muted mb-1"),
            ...
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Input(id="new-user-email", type="email", placeholder="user@royalair.ma", className="mb-1"),
                        dbc.FormText("Must be a valid email address"),
                    ], md=4),
                    dbc.Col([
                        dbc.Label("Password", html_for="new-user-password"),
                        dbc.Input(id="new-user-password", type="password", placeholder="Strong password", className="mb-1"),
                        dbc.FormText("Minimum 8 characters"),
                    ], md=4),
                    dbc.Col([
                        dbc.Label("System Role", html_for="new-user-role"),
                        dbc.Select(id="new-user-role", options=[], placeholder="Choose role"),
                        dbc.FormText("Roles are defined below"),
                    ], md=4),
                ]),
                dbc.Row([dbc.Col(dbc.Button([html.I(className="bi bi-plus-circle me-2"), "Create User"],
                                            id="create-user-btn", color="primary", className="w-100 mt-2"))]),
            ...
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Input(id="new-role-name", placeholder="e.g., perf, ops, finance"),
                    ], md=4),
                    dbc.Col([
                        dbc.Button([html.I(className="bi bi-plus-square me-2"), "Create Role"],
                                   id="create-role-btn", color="secondary", className="mt-4 w-100"),
                    ], md=2),
                    dbc.Col([
                        dbc.Label("Select Role"),
                        dbc.Select(id="perm-role-select", options=[], placeholder="Choose a role"),
                    ], md=3),
                    dbc.Col([
                        dbc.Button([html.I(className="bi bi-trash me-2"), "Delete Role"],
                                   id="delete-role-btn", color="danger", className="mt-4 w-100"),
                    ], md=3),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                        dbc.Label("Allowed Pages for Selected Role"),
                        dcc.Checklist(id="perm-pages-checklist", options=[], value=[], labelStyle={"display": "block"}),
                        dbc.Button([html.I(className="bi bi-save2 me-2"), "Save Permissions"],
                                   id="save-perms-btn", color="primary", className="mt-3"),
                    ], md=6),
                ]),
        ...
        dbc.Card([
            dbc.CardHeader([
                dbc.Row([
                    ...
                    dbc.Col(dbc.Button([html.I(className="bi bi-arrow-clockwise me-2"), "Refresh"],
                                       id="refresh-users-btn", color="secondary", size="sm", className="w-100"), md=2),
