...
from sqlalchemy import Row, delete, insert
...
try:
    from utils_dashboard.utils_page import get_all_metadata_id_pages_dynamic as _valid_nav_ids
except Exception:
...
def get_page_by_name(name: str, session: Session) -> Optional[Page]:
    ...
    if pid is None:
        logging.warning(f"get_page_by_name: unknown page name {name!r} (not in NAV_CONFIG)")
        return None
...
def create_page(page_id: str, session: Session) -> Page:
    ...
    try:
        ...
        if pid is None:
            raise ValueError(f"create_page: page_id must be int-like or a known NAV name, got {page_id!r}")
...
def set_role_pages_by_ids(role_id: int, page_ids: Iterable[int], session: Session, disabled_default: bool = False) -> None:
    """
    ...
    # Clear existing
    session.execute(
        delete(role_page_table).where(role_page_table.c.role_id == role_id)
    )
    ...
    if ids:
        session.execute(
            ...
            [{"role_id": role_id, "page_id": i, "disabled": disabled_default} for i in ids],
        )
